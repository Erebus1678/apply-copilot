...
/**
 * Normalize typography that reads as an AI tell: em/en dashes, curly quotes,
 * and the ellipsis character. Safe to run on partial/streaming text since
 * it's a pure string transform with no structural assumptions.
 */
export function normalizeTypography(text: string): string {
  // Em/en dash used as a sentence break (spaced) becomes a comma; used to
  // join words (unspaced) becomes a spaced hyphen.
